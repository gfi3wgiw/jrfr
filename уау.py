...
# ========== ТВОИ ДАННЫЕ ==========
TOKEN = "8321085396:AAHIffD_Piqzv6CDmBN9hD4CXgLRV7wq94M"
ADMIN_ID = 8423212939
BOT_USERNAME = "StarsEarnBattleBot"  # Вставь сюда юзернейм своего бота (без @)
# =================================
...
battle = {
    ...
    "round_times": [],
    "channels": [],
    "participants": {},  # user_id: {"name": "username", "votes": 0}
    "voted": set(),
    "message_ids": {},
...
async def show_join_menu(message: types.Message):
    ...
    await message.answer(
        ...
        "Каждый может отдать 1 голос за раунд!"
    )
...
async def show_vote_menu(message: types.Message):
    ...
    for uid, data in sorted_ppl:
        text += f"👤 @{data['name']} — {data['votes']} гол.\n"
...
async def show_stats(message: types.Message):
    ...
    for i, (uid, data) in enumerate(sorted_ppl, 1):
        crown = "👑 " if i == 1 else ""
        text += f"{crown}{i}. @{data['name']} — {data['votes']} гол.\n"
...
async def check_subscription(callback: types.CallbackQuery):
    action = callback.data.split("_")[2]  # join или vote
...
async def vote_callback(callback: types.CallbackQuery):
    ...
    if voter_id in battle["voted"]:
        await callback.answer("❌ Ты уже голосовал в этом раунде!", show_alert=True)
        return
...
async def admin_start(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🗳 Голосовать", url=bot_vote_link)]
    ])
...
async def next_round():
    """Переход на следующий раунд и публикация итогов в канал"""
    if not battle["participants"]:
        battle["active"] = False
        for channel_id in battle["message_ids"].keys():
            try:
    ...
    for uid, data in sorted_ppl:
        ...
        text += f"{crown}@{data['name']} — {data['votes']} гол.\n"
    
    if battle["round_num"] >= battle["max_rounds"]:
        # Финальный раунд - объявляем победителя
        text += f"\n🥇 ПОБЕДИТЕЛЬ БАТЛА: @{winner_data['name']}\n"
        text += f"🎁 ВЫИГРЫШ: {battle['stars']} ⭐️ ЗВЁЗД!"
        battle["active"] = False
        
        # Отправляем результаты во все каналы
        for channel_id in battle["message_ids"].keys():
            try:
        ...
        # Не финальный раунд - готовим следующий
        # Отправляем результаты текущего раунда
        for channel_id in battle["message_ids"].keys():
            try:
                await bot.send_message(channel_id, text)
            except:
        ...
        # Обнуляем голоса для нового раунда
